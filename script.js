...
document.addEventListener('DOMContentLoaded', () => {
    const loadingScreen = document.querySelector('.loading-init');
    const newYearMessage = document.querySelector('.new-year-message');
    
    // Simulate loading process
    setTimeout(() => {
      loadingScreen.classList.add('hide'); // Hide loading screen
      newYearMessage.classList.remove('hide'); // Show new year message

      // Remove cursor animation after typing
      setTimeout(() => {
        newYearMessage.style.borderRight = 'none';
      }, 4000); // Matches typing animation duration
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Hiển thị hộp quà sau khi chữ mờ dần và phóng to từ từ
    setTimeout(() => {
      newYearMessage.style.display = "none"; // Ẩn thông điệp sau khi mờ dần
      gift.classList.remove("hidden"); // Hiển thị hộp quà
      gift.classList.add("show"); // Bắt đầu hiệu ứng phóng to
    }, 10000); // Sau 10 giây (3s loading + 4s typing)
  
    // Khi ấn vào hộp quà, hiển thị biểu tượng thách thức
    giftBox.addEventListener("click", () => {
      gift.style.display = "none"; // Ẩn hộp quà
      surprise.classList.remove("hidden"); // Hiển thị biểu tượng thách thức
    });
  });
